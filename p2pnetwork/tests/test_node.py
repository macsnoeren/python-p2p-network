...
class TestNode(unittest.TestCase):
    ...
    def test_node_communication(self):
        ...
        self.assertEqual("node_message:" + node2.id + ":" + node1.id + ":Hi from node 1!", node1_message,
                         "The message is not correctly received by node 2")
        self.assertEqual("node_message:" + node1.id + ":" + node2.id + ":Hi from node 2!", node2_message,
                         "The message is not correctly received by node 1")
    ...
    def test_node_complete(self):
        ...
        if step_1_node_0_total_connections > 0:
            step_1_node_0_connection_node = node_0_connections[0].id + ":" + node_0_connections[0].host + ":" + str(
                node_0_connections[0].port)  # node1.id:127.0.0.1:10001
        ...
        if step_1_node_1_total_connections > 0:
            step_1_node_1_connection_node = node_1_connections[0].id + ":" + node_1_connections[
                0].host  # node0.id:127.0.0.1, inbound port is port of client
        ...
        if step_2_node_2_total_connections > 0:
            step_2_node_2_connection_node = node_2_connections[0].id + ":" + node_2_connections[0].host + ":" + str(
                node_2_connections[0].port)  # node0.id:127.0.0.1:10000
        ...
        if step_2_node_0_total_connections > 0:
            step_2_node_0_connection_node = node_0_connections[0].id + ":" + node_0_connections[
                0].host  # node2.id:127.0.0.1
        ...
        self.assertEqual(step_1_node_0_connection_node, node_1.id + ":127.0.0.1:10001",
                         "Node 0 should be connected (outbound) with node 1.")
        self.assertEqual(step_1_node_1_total_connections, 1, "Node 1 should have one inbound connection.")
        self.assertEqual(step_1_node_1_connection_node, node_0.id + ":127.0.0.1",
                         "Node 1 should be connected (inbound) with node 0.")

        self.assertEqual(step_2_node_2_total_connections, 1, "Node 2 shoud have one outbound connection.")
        self.assertEqual(step_2_node_2_connection_node, node_0.id + ":127.0.0.1:10000",
                         "Node 2 should be connected (outbound) with node 0.")
        self.assertEqual(step_2_node_0_total_connections, 1, "Node 0 should have one inbound connection.")
        self.assertEqual(step_2_node_0_connection_node, node_2.id + ":127.0.0.1",
                         "Node 0 should be connected (inbound) with node 2.")
        ...
        self.assertEqual(message[0], "node_message:" + node_2.id + ":" + node_0.id + ":hello from node 0",
                         "Node 2 should have received a message from node 0")
        self.assertEqual(message[1], "node_message:" + node_1.id + ":" + node_0.id + ":hello from node 0",
                         "Node 1 should have received a message from node 0")
        self.assertEqual(message[2], "node_message:" + node_0.id + ":" + node_1.id + ":hello from node 1",
                         "Node 0 should have received a message from node 1")
        self.assertEqual(message[3], "node_message:" + node_0.id + ":" + node_2.id + ":hello from node 2",
                         "Node 0 should have received a message from node 2")
    ...
    def test_extending_class_of_node(self):
        ...
        if "inbound" in message[3]:
            self.assertEqual(message[3], "inbound_node_connected: " + node1.id,
                             "MyTestNode should have seen this event!")
            self.assertEqual(message[4], "outbound_node_connected: " + node2.id,
                             "MyTestNode should have seen this event!")
        else:
            self.assertEqual(message[4], "inbound_node_connected: " + node1.id,
                             "MyTestNode should have seen this event!")
            self.assertEqual(message[3], "outbound_node_connected: " + node2.id,
                             "MyTestNode should have seen this event!")

        if "outbound" in message[5]:
            self.assertEqual(message[5], "outbound_node_connected: " + node1.id,
                             "MyTestNode should have seen this event!")
            self.assertEqual(message[6], "inbound_node_connected: " + node3.id,
                             "MyTestNode should have seen this event!")
        else:
            self.assertEqual(message[6], "outbound_node_connected: " + node1.id,
                             "MyTestNode should have seen this event!")
            self.assertEqual(message[5], "inbound_node_connected: " + node3.id,
                             "MyTestNode should have seen this event!")

        self.assertEqual(message[7], "node_message from " + node1.id + ": hello from node 1",
                         "MyTestNode should have seen this event!")
        self.assertEqual(message[8], "node_message from " + node1.id + ": hello from node 1",
                         "MyTestNode should have seen this event!")
        self.assertEqual(message[9], "node_message from " + node2.id + ": hello from node 2",
                         "MyTestNode should have seen this event!")
        self.assertEqual(message[10], "node_message from " + node3.id + ": hello from node 3",
                         "MyTestNode should have seen this event!")
