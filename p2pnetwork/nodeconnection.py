...
class NodeConnection(threading.Thread):
    ...
    def send(self, data, encoding_type='utf-8'):
        ...
        if isinstance(data, str):
            try:
                self.sock.sendall( self.main_node._node_send(data.encode(encoding_type), encoding_type) + self.EOT_CHAR )
            ...
            try:
                self.sock.sendall(self.main_node._node_send(json.dumps(data).encode(encoding_type), encoding_type) + self.EOT_CHAR)
            ...
            try:
                self.sock.sendall(self.main_node._node_send(data, encoding_type) + self.EOT_CHAR)
